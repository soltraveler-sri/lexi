...
function VariantPane({ variantKey }: { variantKey: VariantKey }) {
  ...
  return (
    ...
    >
      ...
      <textarea
        className="min-h-32 w-full resize-none rounded-sm bg-transparent font-display text-[17px] leading-[1.6] text-text outline-none placeholder:text-text-faint"
...
export function SidePanelRenderer() {
  ...
  return (
    <aside className="fixed right-0 top-0 z-50 flex h-screen w-[420px] flex-col gap-3 border-l border-border bg-surface p-6 shadow-lg transition-transform duration-150">
      <div className="font-ui text-xs font-medium uppercase tracking-wide text-text-faint">
        Rewrite — pick a variant
      </div>
      <div className="locked-original-node rewrite-strip-active">
